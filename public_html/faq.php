...
$sectionFaqs = [
    'wallet' => [
        ...
        ['How do I create an account?', 'Open the wallet Register page, choose a username, enter your email, enter your phone number for SMS security, and create a strong password. Keep your login details private.'],
    ...
    'burn tracker' => [
        ['Are burns shown before they happen?', 'No. Burn totals and txids come from live node scans of the configured burn address.'],
        ['What if no coins are burned yet?', 'The burn total shows 0.00000000 until real transactions are confirmed to the burn address.'],
    ],
    'docs' => [
...
if ($sectionKey === 'wallet' && $walletSmsEnabled) {
    $sectionFaqs['wallet'][] = ['When does the wallet use SMS codes?', 'Email verification is the first account security step. SMS is the second step for registration, password recovery, and protected wallet actions. Authenticator is an extra double-check if you choose to enable it.'];
    $sectionFaqs['wallet'][] = ['Why am I not receiving SMS codes?', 'SMS delivery depends on the configured SMS provider, carrier filtering, phone number formatting, and A2P/verification status. If SMS is unavailable, open a support ticket so we can help verify the account path.'];
    $sectionFaqs['wallet'][] = ['Can I opt out of SMS?', 'Reply STOP to opt out of SMS messages. Because SMS is the primary account security method, opting out may prevent access to protected login, recovery, or withdrawal features until another approved recovery path is available.'];
