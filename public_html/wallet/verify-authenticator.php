...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $err = wallet_te('wallet.error.verify_auth', [], 'Invalid authenticator code.');
}

render_header('wallet.page.verify_auth.title');
?>
<div class="card">
  <h3><?= h(wallet_te('wallet.page.verify_auth.title', [], 'Authenticator Verification')) ?></h3>
  <p><?= h(wallet_te('wallet.page.verify_auth.intro', [], 'Enter the 6-digit code from your authenticator app.')) ?></p>
  <?php if ($err): ?><p class="err"><?= h($err) ?></p><?php endif; ?>
  <form method="post">
    <input type="hidden" name="csrf_token" value="<?= h(csrf_token()) ?>">
    <label><?= h(wallet_te('wallet.page.verify_auth.label_code', [], 'Authenticator Code')) ?><br><input name="code" inputmode="numeric" pattern="[0-9]{6}" maxlength="6" autocomplete="one-time-code" required autofocus></label><br><br>
    <button type="submit"><?= h(wallet_te('wallet.page.verify_auth.submit', [], 'Verify and Login')) ?></button>
  </form>
</div>
<?php render_footer(); ?>
