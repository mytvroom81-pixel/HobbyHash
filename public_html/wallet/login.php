...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$user) {
        $err = wallet_te('wallet.error.auth_invalid', [], 'Invalid login or temporarily throttled.');
        ...
        try {
            ...
            $err = wallet_te('wallet.error.auth_sms_send', [], 'Password accepted, but SMS verification could not be sent. Please try again.');
        }
...
render_header('wallet.page.login.title');
?>
<div class="card">
  <h3><?= h(wallet_te('wallet.page.login.title', [], 'Login')) ?></h3>
  <?php if ($err): ?><p class="err"><?= h($err) ?></p><?php endif; ?>
  <form method="post">
    <input type="hidden" name="csrf_token" value="<?= h(csrf_token()) ?>">
    <label><?= h(wallet_te('wallet.page.login.label_login', [], 'Email or Username')) ?><br><input name="wallet_login" required autocomplete="section-wallet username"></label><br><br>
    <label><?= h(wallet_te('wallet.page.login.label_password', [], 'Password')) ?><br><input type="password" name="wallet_password" required autocomplete="section-wallet current-password"></label><br><br>
    <button type="submit"><?= h(wallet_te('wallet.page.login.submit', [], 'Login')) ?></button>
  </form>
  <div class="actions"><a class="button" href="<?= h(wallet_url('/forgot-password.php')) ?>"><?= h(wallet_te('wallet.page.login.forgot', [], 'Lost Password?')) ?></a></div>
</div>
<?php render_footer(); ?>
