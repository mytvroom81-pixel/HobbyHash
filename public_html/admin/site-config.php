...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if ($action === 'site_status') {
        ...
        if ($twilioVerifyServiceSid !== '' && !preg_match('/^VA[a-fA-F0-9]{32}$/', $twilioVerifyServiceSid)) {
            $err = 'Twilio Verify Service SID must start with VA and contain 34 characters total.';
        }
...
<div class="admin-grid admin-grid-tight">
  <?= admin_stat_card('Site Mode', str_replace('_', ' ', (string)$siteSettings['site_mode']), $siteSettings['site_mode'] === 'full_launch' ? 'ok' : 'warn') ?>
  <?= admin_stat_card('SMS Service', sms_provider_mode() === 'twilio_verify' ? 'Verify' : 'Manual', sms_is_enabled() ? 'ok' : 'warn') ?>
  <?= admin_stat_card('Admin SMS', ((int)($settings['admin_sms_2fa_required'] ?? 0) === 1) ? 'On' : 'Off', ((int)($settings['admin_sms_2fa_required'] ?? 0) === 1) ? 'ok' : 'warn') ?>
  <?= admin_stat_card('Admin Authenticator', ((int)($settings['admin_totp_required'] ?? 0) === 1) ? 'On' : 'Off', ((int)($settings['admin_totp_required'] ?? 0) === 1) ? 'ok' : 'warn') ?>
  <?= admin_stat_card('Wallet Login SMS', ((int)($settings['wallet_sms_login_required'] ?? 0) === 1) ? 'On' : 'Off', ((int)($settings['wallet_sms_login_required'] ?? 0) === 1) ? 'ok' : 'warn') ?>
  <?= admin_stat_card('Withdrawal SMS', ((int)($settings['wallet_sms_withdrawal_required'] ?? 0) === 1) ? 'On' : 'Off', ((int)($settings['wallet_sms_withdrawal_required'] ?? 0) === 1) ? 'ok' : 'warn') ?>
</div>
...
<div class="admin-card">
  ...
  <p>Email verification is the first account security step. SMS is the second step for wallet registration, login, recovery, and protected wallet actions. Choose whether SMS uses the manual HOBC code sender or Twilio Verify.</p>
  ...
  <form method="post">
    ...
    <label>Twilio Verify Service SID<br>
      <input name="twilio_verify_service_sid" value="<?= h((string)($settings['twilio_verify_service_sid'] ?? '')) ?>" maxlength="64" placeholder="VAxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx">
    </label>
    <p><small>Find this in Twilio Verify &gt; Services &gt; Service settings. It starts with <code>VA</code>.</small></p>
    ...
    <label><input type="checkbox" name="wallet_sms_registration_required" <?= ((int)($settings['wallet_sms_registration_required'] ?? 0) === 1) ? 'checked' : '' ?>> Keep SMS registration enabled for wallet users</label><br>
