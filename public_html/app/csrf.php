...
function csrf_validate_or_fail(): void
{
    ...
    if (!$valid || $created <= 0 || (time() - $created) > $ttl) {
        ...
        if (!headers_sent() && (string)($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
            $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
            $_SESSION['csrf_created_at'] = time();
            $_SESSION['csrf_flash_error'] = 'Security token expired. Please try again.';
            $target = (string)($_SERVER['SCRIPT_NAME'] ?? '/wallet/');
...
function csrf_flash_error(): string
{
    wallet_start_session();
    $message = (string)($_SESSION['csrf_flash_error'] ?? '');
    unset($_SESSION['csrf_flash_error']);
    return $message;
}
