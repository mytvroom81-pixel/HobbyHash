...
share_diff(char *coinbase, const uchar *enonce1bin, const workbase_t *wb, const char *nonce2,
	   const uint32_t ntime32, const char *nonce, const bool use_version_bits,
	   const uint32_t version_bits, uchar *hash, uchar *swap, int *cblen)
{
	unsigned char merkle_root[32], merkle_sha[64];
	uint32_t *data32, *swap32, benonce32;
	uint32_t base_version = 0, rolled_version;
	uchar hash1[32];
	char data[80];
	int i;
	...
	*cblen += wb->coinb2len;
	...
	/* Apply version rolling bits if supplied by the miner. */
	if (use_version_bits) {
		sscanf(wb->bbversion, "%x", &base_version);
		rolled_version = (base_version & ~0x1fffe000U) | (version_bits & 0x1fffe000U);
		data32 = (uint32_t *)data;
		*data32 = htobe32(rolled_version);
	}
...
static void submit_node_block(ckpool_t *ckp, sdata_t *sdata, json_t *val)
{
	...
	if (coinbasehex && cblen && swaphex) {
		...
		/* Fill in the hashes */
		share_diff(coinbase, enonce1bin, wb, nonce2, ntime32, nonce, false, 0, hash, swap, &cblen);
	}
...
static void worker_address_from_workername(const char *workername, char *address, size_t len)
{
	const char *dot;
	size_t addrlen;
	...
	if (!workername || !len)
		return;
	dot = strchr(workername, '.');
	if (dot)
		addrlen = dot - workername;
	else
		addrlen = strlen(workername);
	if (addrlen >= len)
		addrlen = len - 1;
	memcpy(address, workername, addrlen);
	address[addrlen] = '\0';
}

static bool looks_like_hobc_bech32(const char *address)
{
	...
	if (!address)
		return false;
	if (strncmp(address, "hobc1", 5))
		return false;
	data = address + 5;
	if (!*data)
		return false;
	while (*data) {
		if (!strchr(charset, *data))
			return false;
		data++;
	}
	return true;
				      ...
				      const char *workername)
{
	...
	worker_address_from_workername(workername, username, sizeof(username));
			       ...
			       json_t **err_val, int *errnum)
{
	...
	worker_address_from_workername(buf, worker_address, sizeof(worker_address));
	if (ckp->enforce_worker_address) {
		size_t prefixlen = strlen(ckp->worker_address_prefix);

		if (!prefixlen || strncmp(worker_address, ckp->worker_address_prefix, prefixlen)) {
			*err_val = json_string("Invalid HOBC address prefix in workername");
			goto out;
		}
		if (!generator_checkaddr(ckp, worker_address) &&
		    !looks_like_hobc_bech32(worker_address)) {
			*err_val = json_string("Invalid HOBC payout address in workername");
			goto out;
		}
	}
...
static void stratum_send_diff(sdata_t *sdata, const stratum_instance_t *client)
{
	...
	double send_diff = client->diff * sdata->ckp->stratum_diff_scale;

	json_msg = json_object();
	params = json_array();
	json_array_append_new(params, json_real(send_diff));
	json_object_set_new_nocheck(json_msg, "params", params);
	json_object_set_new_nocheck(json_msg, "id", json_null());
	json_object_set_new_nocheck(json_msg, "method", json_string("mining.set_difficulty"));
		       ...
		       const bool submit)
{
	...
	if (fabs(diff - (client->diff * ckp->stratum_diff_scale)) > 0.0000001) {
		client->ssdc = 0;
		return;
	}
	...
	/* Client suggest diff overrides worker mindiff unless pool controls diff */
	if (ckp->ignore_miner_diff)
		mindiff = 0;
...
static double submission_diff(const stratum_instance_t *client, const workbase_t *wb, const char *nonce2,
			      const uint32_t ntime32, const char *nonce, const bool use_version_bits,
			      const uint32_t version_bits, uchar *hash, const bool stale)
{
	char *coinbase;
	uchar swap[80];
	double ret;
	...
	/* Calculate the diff of the share here */
	ret = share_diff(coinbase, client->enonce1bin, wb, nonce2, ntime32, nonce, use_version_bits,
			 version_bits, hash, swap, &cblen);
			    ...
			    const json_t *params_val, json_t **err_val)
{
	...
	ckpool_t *ckp = client->ckp;
	double diff = client->diff * ckp->stratum_diff_scale, wdiff = 0, sdiff = -1;
	...
	const char *version_bits_hex = NULL;
	char idstring[20] = {};
	workbase_t *wb = NULL;
	...
	version_bits_hex = json_string_value(json_array_get(params_val, 5));
	if (version_bits_hex && strlen(version_bits_hex) && validhex(version_bits_hex)) {
		sscanf(version_bits_hex, "%x", &version_bits);
		use_version_bits = true;
	}
	...
	if (id < sdata->blockchange_id)
		stale = true;
	sdiff = submission_diff(client, wb, nonce2, ntime32, nonce, use_version_bits, version_bits,
				hash, stale);
...
out_nowb:
	...
	if (id < client->diff_change_job_id)
		diff = client->old_diff * ckp->stratum_diff_scale;
	if (!invalid) {
		char wdiffsuffix[16];
		...
		if (sdiff >= diff) {
			if (new_share(sdata, hash, id)) {
				LOGINFO("Accepted client %s share diff %.6f/%.6f/%s: %s",
					client->identity, sdiff, diff, wdiffsuffix, hexhash);
				result = true;
			} else {
				err = SE_DUPE;
				json_set_string(json_msg, "reject-reason", SHARE_ERR(err));
				LOGINFO("Rejected client %s dupe diff %.6f/%.6f/%s: %s",
					client->identity, sdiff, diff, wdiffsuffix, hexhash);
				submit = false;
			}
		} else {
			err = SE_HIGH_DIFF;
			LOGINFO("Rejected client %s high diff %.6f/%.6f/%s: %s",
				client->identity, sdiff, diff, wdiffsuffix, hexhash);
			json_set_string(json_msg, "reject-reason", SHARE_ERR(err));
			submit = false;
		}
			 ...
			 const json_t *params_val)
{
	...
	if (ckp->ignore_miner_diff) {
		LOGINFO("Ignoring miner suggested diff from client %s", client->identity);
		return;
	}
			 ...
			 json_t *params_val)
{
	...
	/* Some ASIC firmware sends mining.configure before subscribe/authorise.
	 * Accept it as a compatibility no-op instead of dropping unsubscribed. */
	if (cmdmatch(method, "mining.configure")) {
		json_t *val, *result_val, *ext_arr, *ext_opts;
		size_t i;

		val = json_object();
		result_val = json_object();
		ext_arr = json_array_get(params_val, 0);
		ext_opts = json_array_get(params_val, 1);
		if (ext_arr && json_is_array(ext_arr)) {
			json_t *ext_val;

			json_array_foreach(ext_arr, i, ext_val) {
				const char *ext = json_string_value(ext_val);

				if (!ext)
					continue;
				if (!strcmp(ext, "version-rolling")) {
					const char *mask = NULL;
					int64_t minbit = 0;

					json_object_set_new_nocheck(result_val, "version-rolling",
								    json_true());
					if (ext_opts && json_is_object(ext_opts)) {
						mask = json_string_value(json_object_get(ext_opts,
									 "version-rolling.mask"));
						minbit = json_integer_value(json_object_get(ext_opts,
					...
					json_object_set_new_nocheck(result_val, "version-rolling.mask",
								    json_string(mask));
					json_object_set_new_nocheck(result_val, "version-rolling.min-bit-count",
								    json_integer(minbit));
					continue;
				}
				if (!strcmp(ext, "subscribe-extranonce")) {
					json_object_set_new_nocheck(result_val, "subscribe-extranonce",
								    json_true());
					continue;
				}
				/* Explicitly reject unknown extensions */
				json_object_set_new_nocheck(result_val, ext, json_false());
			}
		}
		...
		stratum_add_send(sdata, val, client_id, SM_NONE);
		return;
	}
