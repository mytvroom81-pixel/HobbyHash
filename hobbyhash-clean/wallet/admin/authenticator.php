...
<div class="card">
  ...
  <div class="admin-actions">
    <?php if ((int)($adminRow['totp_enabled'] ?? 0) !== 1): ?>
      <button type="button" data-open-modal="admin-totp-setup-modal"><?= $secret !== '' ? 'Continue Setup' : 'Set Up Authenticator' ?></button>
    <?php else: ?>
...
<div class="modal-backdrop <?= ($secret !== '' && (int)($adminRow['totp_enabled'] ?? 0) !== 1) ? 'is-open' : '' ?>" id="admin-totp-setup-modal" aria-hidden="<?= ($secret !== '' && (int)($adminRow['totp_enabled'] ?? 0) !== 1) ? 'false' : 'true' ?>">
  <div class="modal-card authenticator-modal" role="dialog" aria-modal="true" aria-labelledby="admin-totp-setup-title">
    ...
    <?php if ($secret === ''): ?>
      <p>Start by generating a new setup QR code. After that, scan it with your authenticator app and confirm the first 6-digit code.</p>
      <form method="post">
