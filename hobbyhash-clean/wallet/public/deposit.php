    ...
    try {
        if ($action === 'generate') {
            if ((int)$settings['deposits_paused'] === 1) {
                $err = 'Deposits are currently paused.';
            } else {
            ...
            if ($stmt->rowCount() === 0) {
                throw new RuntimeException('Receive wallet not found.');
            }
            $ok = 'Receive wallet name updated.';
        } else {
            $err = 'Unknown receive wallet action.';
        }
    } catch (Throwable $e) {
        wallet_log_error('receive wallet action failed: ' . $e->getMessage());
        if ($err === '') {
            $err = 'Receive wallet action failed.';
        }
    }
}
...
render_header('Receive');
?>
<div class="card">
  <h3>Receive HOBC</h3>
  <p><b>Total Wallet Balance:</b> <span id="live-wallet-balance"><?= h($walletBalance) ?></span> HOBC</p>
  <p>Receive wallets are labels for organizing incoming payments. They do not have separate spendable balances.</p>
  <?php if ($err): ?><p class="err"><?= h($err) ?></p><?php endif; ?>
  <?php if ($ok): ?><p class="ok"><?= h($ok) ?></p><?php endif; ?>
  <form method="post">
    <input type="hidden" name="csrf_token" value="<?= h(csrf_token()) ?>">
    <input type="hidden" name="action" value="generate">
    <label>New Receive Wallet Name<br><input name="label" maxlength="80" placeholder="Mining, Savings, Exchange deposits"></label>
    <button type="submit">Create Receive Wallet</button>
  </form>
</div>

<div class="card">
  <h3>Your Receive Wallets</h3>
  <table>
    <tr><th>Name</th><th>Address</th><th>Total Received</th><th>Pending</th><th>Deposits</th><th>Actions</th></tr>
      ...
      <tr>
        <td>
          <span class="wallet-name-cell">
            ...
            <button type="button" class="small-button edit-wallet-button" data-address-id="<?= h((string)$row['id']) ?>" data-label="<?= h(wallet_edit_receive_label((string)($row['label'] ?? ''))) ?>">Edit</button>
          </span>
        </td>
        <td>
          <?= wallet_address_text((string)$row['address']) ?>
          <button type="button" class="small-button qr-small-button show-qr-button" data-address-id="<?= h((string)$row['id']) ?>" data-address="<?= h((string)$row['address']) ?>" data-label="<?= h(wallet_display_receive_label((string)($row['label'] ?? ''))) ?>">QR</button>
        </td>
        ...
        <td><button type="button" class="small-button view-wallet-transactions" data-address-id="<?= h((string)$row['id']) ?>">View transactions</button></td>
      </tr>
    <?php endforeach; ?>
    </tbody>
  </table>
...
<div class="modal-backdrop" id="edit-wallet-modal" aria-hidden="true">
  <div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="edit-wallet-title">
    <div class="modal-header">
      <div>
        <span class="eyebrow">Receive wallet</span>
        <h3 id="edit-wallet-title">Edit Wallet Name</h3>
      </div>
    ...
    <form method="post">
      ...
      <label>Wallet Name<br><input name="label" id="edit-wallet-label" maxlength="80" required></label>
      <button type="submit">Save Wallet Name</button>
    </form>
  </div>
</div>
...
<div class="modal-backdrop" id="receive-transactions-modal" aria-hidden="true">
  <div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="receive-transactions-title">
    <div class="modal-header">
      <div>
        <span class="eyebrow">Receive wallet</span>
        <h3 id="receive-transactions-title">Wallet Transactions</h3>
      </div>
      <button type="button" class="small-button modal-close" data-close-modal>Close</button>
    </div>
...
<div class="modal-backdrop" id="qr-code-modal" aria-hidden="true">
  <div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="qr-code-title">
    <div class="modal-header">
      <div>
        <span class="eyebrow">Receive wallet</span>
        <h3 id="qr-code-title">Receive QR Code</h3>
      </div>
      <button type="button" class="small-button modal-close" data-close-modal>Close</button>
    </div>
    <p id="qr-code-label"></p>
    <div class="qr-code-wrap"><img id="qr-code-image" src="" alt="Receive address QR code"></div>
...
<div class="card">
  <h3>Transactions for <?= h(wallet_display_receive_label((string)($selectedAddress['label'] ?? ''))) ?></h3>
  <p><?= wallet_address_text((string)$selectedAddress['address']) ?></p>
  <table>
    <tr><th>TXID</th><th>Amount</th><th>Confs</th><th>Status</th><th>Detected</th></tr>
    ...
    <?php endforeach; ?>
    <?php if (!$detailsRows): ?>
      <tr><td colspan="5">No external deposits found for this receive wallet yet.</td></tr>
    <?php endif; ?>
  </table>
...
(() => {
  ...
  const openReceiveTransactions = (id) => {
    ...
    receiveSummary.innerHTML = [
      detailRow('Wallet', esc(wallet.label)),
      detailRow('Address', addressText(wallet.address)),
      detailRow('Transactions', esc(String(wallet.transactions.length))),
    ].join('');
    if (!wallet.transactions.length) {
      receiveTransactions.innerHTML = '<p>No external deposits found for this receive wallet yet.</p>';
    } else {
  ...
  const openQrModal = (id, address, label) => {
    ...
    qrLabel.innerHTML = `<b>${esc(label || 'Receive address')}</b>`;
          ...
          `<tr>
            <td>
              <span class="wallet-name-cell">
                ...
                <button type="button" class="small-button edit-wallet-button" data-address-id="${esc(row.id)}" data-label="${esc(row.edit_label || '')}">Edit</button>
              </span>
            </td>
            <td>${addressText(row.address)} <button type="button" class="small-button qr-small-button show-qr-button" data-address-id="${esc(row.id)}" data-address="${esc(row.address)}" data-label="${esc(row.label)}">QR</button></td>
            <td>${esc(row.total_received)}</td>
            <td>${esc(row.pending_received)}</td>
            <td>${esc(row.deposit_count)}</td>
            <td><button type="button" class="small-button view-wallet-transactions" data-address-id="${esc(row.id)}">View transactions</button></td>
          </tr>`
        ).join('');
        bindEditButtons();
      }
